...
*
*   원래는 스프링 MVC 아키텍처에서는
*       CONTROLLER가 VIEW 반환하는 작업진행
*       - 문제점 : 리액트와 스프링이 통합되었을 때
*           리액트 라우터( Link, get ) , 스프링( get )
*   - 스프링 안에 리액트가 포함되므로 get요청 시 스프링 매핑 우선
*   - 문제해결 : get요청 view 찾을 때 controller가 resources가서 찾으라고 설정
*
*   // 1. 스프링 설정 클래스 생성
*   // 2. extends WebMvcConfigurerAdapter : MVC 아키텍처 연결 설정 변경
*   // 3. 오른쪽 클릭 -> 생성 -> 오버라이딩
*   // 4. addViewControllers 메소드 오버라이딩하기
* */
